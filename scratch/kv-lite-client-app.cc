...
void KvLiteClientApp::ProcessReceive(const KvLiteResponse &rsp)
{
    ...
    switch (msgType) {
        ...
        case KvLiteMsgType::DATA: {
            ...
            m_seenRespIds.clear();
            m_sentHandshakeReqIds.clear();
            // Compute next window slot reqId: base + (completed % window) + rounds*window
            uint64_t rounds = m_completedRequests / m_maxWindows;
            uint32_t slot = m_completedRequests % m_maxWindows;
            uint64_t nextReqId = m_baseReqId + slot + (rounds * m_maxWindows);
                KvLiteRequest next;
